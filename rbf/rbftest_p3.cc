...
<<<<<<< HEAD
	...
	int nullFieldsIndicatorActualSize = getActualByteForNullsIndicator(
			recordDescriptorForTwitterUser.size());
	unsigned char *nullsIndicator = (unsigned char *) malloc(
			nullFieldsIndicatorActualSize);
	memset(nullsIndicator, 0, nullFieldsIndicatorActualSize);

	int nullFieldsIndicatorActualSize2 = getActualByteForNullsIndicator(
			recordDescriptorForTweetMessage.size());
	unsigned char *nullsIndicator2 = (unsigned char *) malloc(
			nullFieldsIndicatorActualSize2);
	...
	for (int i = 0; i < numRecords / batchSize; i++) {
		for (int j = 0; j < batchSize; j++) {
			...
			prepareLargeRecordForTwitterUser(
					recordDescriptorForTwitterUser.size(), nullsIndicator,
					i * batchSize + j, record, &size);

			rc = rbfm->insertRecord(fileHandle, recordDescriptorForTwitterUser,
					record, rid);
			assert(
					rc == success
							&& "Inserting a record for the file #1 should not fail.");
		...
		for (int j = 0; j < batchSize; j++) {
			...
			prepareLargeRecordForTweetMessage(
					recordDescriptorForTweetMessage.size(), nullsIndicator2,
					i * batchSize + j, record2, &size2);

			rc = rbfm->insertRecord(fileHandle2,
					recordDescriptorForTweetMessage, record2, rid2);
			assert(
					rc == success
							&& "Inserting a record  for the file #2 should not fail.");

			rids2.push_back(rid2);
		}

		if (i % 5 == 0 && i != 0) {
			cout << i << " / " << numRecords / batchSize << " batches ("
					<< numRecords << " records) inserted so far for both files."
					<< endl;
		}
	}

	cout << "Inserting " << numRecords << " records done for the both files."
			<< endl << endl;
	...
	if (fsize == 0) {
		...
		cout << "***** [FAIL] Test Case Private 3 Failed! *****" << endl
				<< endl;
		return -1;
	}
	...
	ofstream ridsFile("test_private_3a_rids",
	...
	ofstream ridsFile2("test_private_3b_rids",
			ios::out | ios::trunc | ios::binary);
	...
	cout
			<< "***** RBF Test Case Private 3 Finished. The result will be examined. *****"
			<< endl << endl;
